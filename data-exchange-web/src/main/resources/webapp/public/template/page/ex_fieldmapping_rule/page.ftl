    ...
    <script>
        layui.use(['laydate', 'table', 'layer', 'global'], function () {
            ...
            var dataTable = table.render({
                elem: '#dataTable',
                url: '/exFieldmappingRule/pageData', //数据接口
                where: {},
                page: true, //开启分页
                cols: [[ //表头
                    {type: 'checkbox'},
                    {field: 'sourceDbtype', title: '源数据库类型'},
                    {field: 'destDbtype', title: '目标数据库类型'},
                    {field: 'sourceFieldtype', title: '字段源类型'},
                    {field: 'destFieldtype', title: '字段目标类型'},
                    {field: 'rule', title: '规则'},
                    {field: 'id', title: ''},
                    {
            ...
            table.on('tool(dataTable)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') { //详情
                    $.ajax({
                        url: '/exFieldmappingRule/detailPage',
                        data: {id: data.id},
                        async: false,
                    ...
                    $.ajax({
                        url: '/exFieldmappingRule/updatePage',
                        data: {id: data.id},
                        async: false,
                        success: function (data, status, xhr) {
                            layer.open({
            ...
            $('#newItem').click(function () {
                $.ajax({
                    url: '/exFieldmappingRule/newPage',
                    async: false,
                    success: function (data, status, xhr) {
                        layer.open({
                            type: 1,
            ...
            $('#delItems').click(function () {
                ...
                layer.confirm('确认批量删除？', function (index) {
                    $.ajax({
                        url: '/exFieldmappingRule/deleteBatch',
