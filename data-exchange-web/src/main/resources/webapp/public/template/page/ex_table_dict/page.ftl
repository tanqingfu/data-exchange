    ...
    <script>
        layui.use(['laydate', 'table', 'layer', 'global'], function () {
            ...
            var dataTable = table.render({
                elem: '#dataTable',
                url: '/exTableDict/pageData', //数据接口
                where: {},
                page: true, //开启分页
                cols: [[ //表头
                    {type: 'checkbox'},
                    {field: 'syncId', title: '同步表id'},
                    {field: 'dbDesc', title: '库描述'},
                    {field: 'dbTable', title: '表名'},
...
//                    {field: 'modifyTime', title: '修改时间'},
            ...
            table.on('tool(dataTable)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') { //详情
                    $.ajax({
                        url: '/exTableDict/detailPage',
                        data: {syncId: data.syncId},
                        async: false,
            ...
            $('#delItems').click(function () {
                ...
                layer.confirm('确认批量删除？', function (index) {
                    $.ajax({
                        url: '/exTableDict/deleteBatch',
                        data: {syncIds: data.map(item => item.syncId)},
                        async: false,
                        traditional: true,
                        success: function (data, status, xhr) {
                            if (data.success) {
