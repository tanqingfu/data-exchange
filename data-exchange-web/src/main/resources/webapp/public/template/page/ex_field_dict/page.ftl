        ...
        <div class="layui-card-body">
            <form class="layui-form" id="queryForm">
                <div class="layui-form-item">
                    <label class="layui-form-label" style="width: auto">同步表名</label>
                    <div class="layui-input-inline">
                        <input type="text" name="dbTable" class="layui-input">
                    </div>
    ...
    <script>
        layui.use(['laydate', 'table', 'layer', 'global'], function () {
            ...
            var dataTable = table.render({
                elem: '#dataTable',
                url: '/exFieldDict/pageData', //数据接口
                where: {},
                page: true, //开启分页
                cols: [[ //表头
                    {type: 'checkbox'},
                    {field: 'dbTable', title: '同步表名'},
                    {field: 'syncSeqno', title: '序号'},
                    {field: 'syncField', title: '字段'},
                    {field: 'syncType', title: '字段类型'},
                    {
            ...
            table.on('tool(dataTable)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') { //详情
                    $.ajax({
                        url: '/exFieldDict/detailPage',
                        data: {syncSeqno: data.syncSeqno},
                        async: false,
            ...
            $('#delItems').click(function () {
                ...
                layer.confirm('确认批量删除？', function (index) {
                    $.ajax({
                        url: '/exFieldDict/deleteBatch',
                        data: {syncSeqnos: data.map(item => item.syncSeqno)},
                        async: false,
                        traditional: true,
                        success: function (data, status, xhr) {
                            if (data.success) {
