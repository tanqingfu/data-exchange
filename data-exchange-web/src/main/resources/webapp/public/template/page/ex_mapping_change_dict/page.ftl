    ...
    <script>
        layui.use(['laydate', 'table', 'layer', 'global'], function () {
            ...
            var dataTable = table.render({
                elem: '#dataTable',
                url: '/exMappingChangeDict/pageData', //数据接口
                where: {},
                page: true, //开启分页
                cols: [[ //表头
                    {type: 'checkbox'},
            ...
            table.on('tool(dataTable)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') { //详情
                    $.ajax({
                        url: '/exMappingChangeDict/detailPage',
                        data: {id: data.id},
                        async: false,
                    ...
                    layer.confirm('确定要删除该条数据吗？', function (index) {
                        $.ajax({
                            url: '/exMappingChangeDict/delete',
                            data: {id: data.id},
                            async: false,
                            success: function (data, status, xhr) {
                                if (data.success) {
                    ...
                    $.ajax({
                        url: '/exMappingChangeDict/updatePage',
                        data: {id: data.id},
                        async: false,
                        success: function (data, status, xhr) {
                            layer.open({
            ...
            $('#newItem').click(function () {
                $.ajax({
                    url: '/exMappingChangeDict/newPage',
                    async: false,
                    success: function (data, status, xhr) {
                        layer.open({
                            type: 1,
            ...
            $('#delItems').click(function () {
                ...
                layer.confirm('确认批量删除？', function (index) {
                    $.ajax({
                        url: '/exMappingChangeDict/deleteBatch',
