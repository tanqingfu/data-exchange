    ...
    <script>
        layui.use(['laydate', 'table', 'layer', 'global'], function () {
            ...
            var dataTable = table.render({
                elem: '#dataTable',
                url: '/exOrganizationLevel/pageData', //数据接口
                where: {},
                page: true, //开启分页
                cols: [[ //表头
                    {type: 'checkbox'},
                    {field: 'orgLevel', title: '机构id'},
                    {field: 'orgDesc', title: '机构层级信息'},
                    {
            ...
            table.on('tool(dataTable)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') { //详情
                    $.ajax({
                        url: '/exOrganizationLevel/detailPage',
                        data: {id: data.id},
                        async: false,
                    ...
                    layer.confirm('确定要删除该条数据吗？', function (index) {
                        $.ajax({
                            url: '/exOrganizationLevel/delete',
                            data: {id: data.id},
                            async: false,
                            success: function (data, status, xhr) {
                                if (data.success) {
                    ...
                    $.ajax({
                        url: '/exOrganizationLevel/updatePage',
                        data: {id: data.id},
                        async: false,
                        success: function (data, status, xhr) {
                            layer.open({
            ...
            $('#newItem').click(function () {
                $.ajax({
                    url: '/exOrganizationLevel/newPage',
                    async: false,
                    success: function (data, status, xhr) {
                        layer.open({
                            type: 1,
            ...
            $('#delItems').click(function () {
                ...
                layer.confirm('确认批量删除？', function (index) {
                    $.ajax({
                        url: '/exOrganizationLevel/deleteBatch',
