layui.define(['laytpl', 'layer'], function (e) {
    "use strict";
    layui.link('/static/module/autocomplete.css');
    var hint = layui.hint(),
        $ = layui.jquery,
        laytpl = layui.laytpl,
        layer = layui.layer,
        ...
        if (_config.url) {
            (!_config.cache && _config.ajax[_self.index] != undefined) && _config.ajax[_self.index].abort(), _config.ajax[_self.index] = $.ajax({
                ...
                success: function (resp) {
                    return 0 != eval('resp.' + _config.response.code) ?
                        layer.msg(eval('resp.' + _config.response.data)) :
                        _config.data[_self.index] = eval('resp.' + _config.response.data), _self.renderData(_config.data[_self.index])
                },
                error: function () {
                    hint.error("请求失败")
                },
