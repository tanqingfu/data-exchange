    ...
    <script>
        layui.use(['laydate', 'table', 'layer', 'global'], function () {
            ...
            var dataTable = table.render({
                elem: '#dataTable',
                url: '/${table.entityPath}/pageData', //数据接口
                where: {},
                page: true, //开启分页
                cols: [[ //表头
                    {type: 'checkbox'},
<#list table.fields as field>
                    {field: '${field.propertyName}', title: '${field.comment!field.propertyName}'},
</#list>
                    {
            ...
            table.on('tool(dataTable)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') { //详情
                    $.ajax({
                        url: '/${table.entityPath}/detailPage',
                        data: {id: data.id},
                        async: false,
                    ...
                    $.ajax({
                        url: '/${table.entityPath}/updatePage',
                        data: {id: data.id},
                        async: false,
                        success: function (data, status, xhr) {
                            layer.open({
            ...
            $('#delItems').click(function () {
                ...
                layer.confirm('确认批量删除？', function (index) {
                    $.ajax({
                        url: '/${table.entityPath}/deleteBatch',
